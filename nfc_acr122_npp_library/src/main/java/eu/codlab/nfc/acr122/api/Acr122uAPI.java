...
public class Acr122uAPI {
	private CardTerminal _terminal;
	private CardChannel _ch;
	private boolean _debugMode;

	public Acr122uAPI(CardTerminal terminal, CardChannel ch, boolean debugMode){
	 ...
	 * @return
	 */
	public boolean setLED(int led_state_control_mask,
		...
		/* construct the command */
		try {
			state = transceive_apdu(cmd).getSW1() == 0x90;
		} catch (CardException e) {
			e.printStackTrace();
		}
		return state;
	...
	public boolean setBuzzerState(boolean activate){

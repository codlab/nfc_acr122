...
public class TamaRequestEncoder {
	...
	public <T> byte[] encodeMessage(T request) throws TamaException {
        ...
        else if (request instanceof InListPassiveTargetReqPoll)
            return inListPassiveTargetPolling((InListPassiveTargetReqPoll)request);
    ...
    private byte[] inListPassiveTarget(InListPassiveTargetReq request) {
        byte[] buffer = new byte[4 + request.getInitiatorDataLength()];//4
        buffer[0] = (byte)0xD4;
        buffer[1] = (byte)0x4A;
	...
	private byte[] inListPassiveTargetPolling(InListPassiveTargetReqPoll request) {
		byte[] buffer = new byte[5 + request.getInitiatorDataLength()];//4
		buffer[0] = (byte)0xD4;
		buffer[1] = (byte)0x60;;//(byte)0x4A;
		buffer[2] = (byte)0xFF;//request.getMaxTargets();
		buffer[3] = (byte)0x01;//request.getBateRate();
        buffer[4] = (byte)0x10;//request.getBateRate();
		if (request.hasInitiatorData()) {
			System.arraycopy(request.getInitiatorData(), 0, buffer, 4, request.getInitiatorDataLength());
		}
		return buffer;
